...
class login extends REST_Controller{
	...
	{
		$data = $this->post();
		if(array_key_exists('CI_Admin',$data)&&array_key_exists('username', $data)&& array_key_exists('contraseña',$data))
		{

			$CI_admin = $this->post("CI_Admin");
			$username = $this->post("username");
			$Contraseña = $this->post("contraseña");
			$Contraseñamd5 = md5($Contraseña);

			$login = $this->UsAdministrativo_model->verificar_login($CI_admin, $username, $Contraseñamd5);
			if($login)
			{
				if($login[0]->estadouser == 'AC')
				{
					...
					$date = new DateTime();
					$tokenData['id_usuarioad']= $login[0] -> id_usuarioad;
